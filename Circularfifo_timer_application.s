...
;THIS PROGRAM DEALS WITH THE APPLICATION OF THE CIRCULAR FIFO for simulating a timer.
...
;>>>>> begin main program code <<<<<
			...
			BL Init_PIT_IRQ  
			; load runstopwatch and store zero in it
			LDR R0,=RunStopWatch
			MOVS R1,#0
			STRB R1,[R0,#0]
			;load count and load zero
			LDR R0,=Count
			MOVS R1,#0
			STR R1,[R0,#0]
...
SWITCH	    CMP R0,#'C'      ;compares if clear command
			BEQ OUTPUT1
			B switch1
			
			
OUTPUT1 	
			BL PutChar         ; dequeue command instructions
			MOVS R0,#0x0D      ; carriage return character
			BL PutChar
			MOVS R0,#0x0A       ; newline character
			BL PutChar
...
switch1     
			CMP R0,#'D' ; compares if enqueue commmand
			BEQ OUTPUT2
			B SWITCH2			
...
SWITCH3		CMP R0,#'P'     ; PAUSE THE TIMER 
			BNE SWITCH4
...
SWITCH4		CMP R0,#'T'       ; SET STOPWATCH RUNVARIABLE TO 1
			BNE STEP6 
...
;RUNS THE PIT TIMER	
;-----------------------------
PIT_ISR                CPSID I      ;MASK INTERRUPTS
					   LDR R0,=RunStopWatch       ;;STARTS THE WATCH
					   LDRB R0,[R0,#0]
					   CMP R0,#0              
					   BNE COUNTER_INCREMENT
					   B PIT_ISR_STOP    ;COUNT UNCHANGED
					  
			   

COUNTER_INCREMENT    LDR R0,=Count
					 LDR R1,[R0,#0]
					 ADDS R1,R1,#1
					 STR R1,[R0,#0]
					 

PIT_ISR_STOP          LDR R0,=PIT_CH0_BASE
